...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        Integer elements = addToRow(smallest, value);
        Cell bump = smallest;
        while(elements != null){
            if(bump.below == null){
                Cell newRow = bump.below;
                newRow = new Cell (elements);
                bump.below = newRow;
                newRow.above = bump;
                return;
            }else{
                bump = bump.below;
                elements = addToRow(bump, elements);
            }
        }
    }
    ...
    protected Integer addToRow(Cell curr, int value) {
        int bump  =0;
        // curr is smaller keep sorting
        while(curr.value <  value){
            if(curr.right == null){
                Cell last = new Cell (value);
                curr.right = last;
